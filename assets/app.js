...
function renderMarket(m){
  ...
  $('#marketStatus').textContent = m.status || 'Auto Update';
...
function renderRates(r){ $('#rates').innerHTML=(r.rates||[]).map(x=>`<div class="rate-item"><div><div class="rate-name">${x.name}</div><div class="rate-value">${fmt(x.value,3)}%</div></div><div class="rate-change ${cls(x.change_bps)}">${sign(x.change_bps)}${fmt(x.change_bps,1)} bps</div></div>`).join('') || '<div class="empty">利率資料尚未更新。</div>'; }
